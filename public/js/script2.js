...
const WeeklySet = {
	...
	*/
	printMenus : function(setNm) {
		...
		$.ajax({
			url : "/fly&go/menus2.php",//폴더 밖에서꺼낸것 
			type : "get",
			dataType : "html",
			data : { setNm : setNm },
			success : function(res) {
				...
				WeeklySet.updateSelection(); // (오른쪽)메뉴 변경시 선택 항목 업데이트 
			},
			error : function (err) {
				console.error(err);
			}
	...
	*/
	checkMaxSelection : function() {
		...
		if (selectedCnt > maxCnt) {
			const msg = `                                           [ Fly & Go noctice ]
                            선택옵션은 최대 ${maxCnt}개 선택 가능합니다.`;
			throw msg;
		}
	},
	/**
	...
	*/
	popupViewMore : function (num) {
		...
		if ($("#layer").length == 0) { // 레이어가 없는 경우에만 추가 ! 1이상인경우
										// 밑에꺼추가해줄필요없음 
										//그래서 밑에 숫자가 1/10이런식으로 출력됨
...
$(() => {
	...
	$("body").on("click", ".menus input[name^='menu']", function(e) {
		...
		WeeklySet.updateSelection();//현재선택된것 업데이트
	});
